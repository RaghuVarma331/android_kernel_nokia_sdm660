...
git clone https://github.com/RaghuVarma331/clang.git -b android-9.0 --depth=1 clang
cd kernel
clear
...
cd DRG_sprout
zip -r Black_caps-9.0-CLANG-FIH-SDM660-2018-$(date +"%Y%m%d").zip META-INF patch tools Image.gz-dtb anykernel.sh
cp -r Black_caps-9.0-CLANG-FIH-SDM660-2018-$(date +"%Y%m%d").zip $path/black_caps
cd
cd $path
cd black_caps
