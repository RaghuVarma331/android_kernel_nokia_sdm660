...
static unsigned char _check_channel_min_value(void)
{
	...
	for (i = 0; i < sys.sc_sensor_num * sys.sc_driver_num; i++) {
		...
		if (current_data_temp[i] < min_limit_value[i]) {
			...
			test_result = 0;
			//pr_err("current[%d]%d,limit[%d]%d",i,current_data_temp[i],i,min_limit_value[i]);
		}
	}
...
static unsigned char _check_area_accord_1143(void)
{
	...
	for (i = 0; i < sys.sc_sensor_num; i++) {
		for (j = 0; j < sys.sc_driver_num; j++) {
			...
			accord_temp = 0;
//                      pr_err("need_check[%d]%d.",index,need_check[index]);
			if (need_check[index] == _NEED_NOT_CHECK) {
				continue;
			}
...
//                                      pr_err("buf[%d]=%d,buf[%d]=%d,buf[%d]=%d,buf[%d]=%d",i + 1 + j * sys.sc_sensor_num,current_data_temp[i + 1 + j * sys.sc_sensor_num],index,current_data_temp[index],
...
#endif
	{
		for (j = 0; j < sys.sc_sensor_num * sys.sc_driver_num; j++) {
			if (shake_value[j] >= permit_jitter_limit) {
				...
				test_result = 0;
				pr_err("point %d beyond jitter limit", j);
			}
		}
	}
	return test_result;
...
static unsigned char _check_uniformity(void)
{
	...
	pr_err("min_val: %d, max_val: %d, tp uniformity(x1000): %lx", min_val, max_val, uniformity);
...
static s32 _check_device_version(int type)
{
	...
	if (type & _VER_EQU_CHECK) {
		...
		pr_err("ini version1:%s\n", ini_version1);
		pr_err("ini version2:%s\n", ini_version2);
...
static unsigned char _rawdata_test_result_analysis(int check_types)
{
	...
	if (((check_types & _UNIFORMITY_CHECK) != 0) && ((test_error_code & _BEYOND_UNIFORMITY_LIMIT) != 0)) {
		pr_err("beyond_uniformity_limit_num:%d", beyond_uniformity_limit_num);
	...
	pr_err("test_end:0x%0x err:0x%0x", test_end, err);
...
//                      pr_err("accord_line_limit[%d]=%ld",i,accord_line_limit[i]);
	...
	pr_err("1143 accord test_error_code 0x%x", test_error_code);
	return 1;
}
...
//                      pr_err("need_check[%d]%d",point-1,need_check[point-1]);
		}
...
static s32 _check_rawdata_proc(int check_types, u16 * data, int len, char *save_path)
{
	...
	if ((check_types & _OFFSET_CHECK) != 0) {
		...
		pr_err("After offset check:%ld", offset_limit);
	...
	if ((check_types & _UNIFORMITY_CHECK) != 0) {
		...
		pr_err("After uniformity check:%ld\n", uniformity_limit);
...
//  if ((check_types & _TEST_RESULT_SAVE) != 0)
	...
	//pr_err("The %d group rawdata test end!\n", current_data_index);
	...
	if ((check_types & _JITTER_CHECK) != 0) {
		...
		pr_err("After FullScreenJitterCheck\n");

	}

	if (_rawdata_test_result_analysis(check_types) == 0) {

		pr_err("After TestResultAnalyse\n");
...
static int _hold_ss51_dsp(void)
{
	...
	while (retry++ < 200) {
		...
		if (ret <= 0) {
			...
			pr_err("Hold ss51 & dsp I2C error,retry:%d", retry);
		...
		if (0x0C == rd_buf[0]) {
			...
			pr_err("Hold ss51 & dsp confirm SUCCESS");

			break;

		}

		pr_err("Hold ss51 & dsp confirm 0x4180 failed,value:%d", rd_buf[0]);
...
//                                      pr_err("tmp[%d]%d,offest %d",index -1,tmp[index -1],offest);
					break;
				}
			}
		}
...
gt900_short_again:
	...
	/*
	 * pr_err("AVDD:%d",sys_avdd);
	 pr_err("gt900_short_threshold:%d",gt900_short_threshold);
	 pr_err("gt900_resistor_warn_threshold:%d",gt900_resistor_warn_threshold);
	 pr_err("gt900_drv_drv_resistor_threshold:%d",gt900_drv_drv_resistor_threshold);
	 pr_err("gt900_drv_sen_resistor_threshold:%d",gt900_drv_sen_resistor_threshold);
	 pr_err("gt900_sen_sen_resistor_threshold:%d",gt900_sen_sen_resistor_threshold);
	 pr_err("gt900_drv_gnd_resistor_threshold:%d",gt900_drv_gnd_resistor_threshold);
	 pr_err("gt900_sen_gnd_resistor_threshold:%d",gt900_sen_gnd_resistor_threshold);
	 */
	memset(tmp, 0, g_size);
	...
	if (short_test_cnt < gt900_short_test_times) {
		pr_err("short test again %d", short_test_cnt);
...
gt900_test_exit:
	...
	short_test_end();
	free(data);
	pr_err("short upload_cnt:%d", short_result[0]);

	pr_err("short test end,ret 0x%x", ret);
...
static s32 _get_test_parameters(char *inipath)
{
	...
	if (test_types & _ACCORD_CHECK) {
		...
		pr_err("accord_limit:%ld", accord_limit_tmp);
	...
	if (test_types & _OFFSET_CHECK) {
		...
		pr_err("offset_limit:%ld", offset_limit);
	...
	if (test_types & _SPECIAL_CHECK) {
		...
		pr_err("special_limit:%ld", special_limit);
	...
	if (test_types & _VER_EQU_CHECK) {
		...
		pr_err("version_equ:%s", ini_version1);
		...
		pr_err("version_greater:%s", ini_version1);
		...
		pr_err("version_between1:%s", ini_version1);

		ini_read(inipath, (const char *)"version_between2", (char *)ini_version2);

		pr_err("version_between2:%s", ini_version2);
	...
	if (test_types & _MODULE_SHORT_CHECK) {
		...
		if (ret > 0) {
			...
			pr_err("gt900_short_test_times:%d", gt900_short_test_times);
		...
		if (ret > 0) {
			...
			pr_err("tri_pattern_r_ratio:%ld", tri_pattern_r_ratio);
	...
	if (uniformity_limit > 0) {
		...
		pr_err("uniformity_limit:%ld", uniformity_limit);

	}

	pr_err("_get_test_parameters success!");
...
static s32 _init_test_paramters(char *inipath)
{
	...
	pr_err("begin _get_test_parameters");
#if GTP_TEST_PARAMS_FROM_INI
...
*/
	...
	pr_err("module_cfg len %d ,config len %d", ret, sys.config_length);

	if (ret < 0 || ret != sys.config_length) {

		pr_err("switch the original cfg.");
...
static int _check_config(void)
{
	...
	}else
	{
		for(i = 0; i < sys.config_length; i++)
		{
			pr_err("module[%d]0x%02X, config[%d]0x%02X",i,module_cfg[i],i,config[i]);
		}
	}
...
TEST_START:
	...
	pr_err("sen*drv:%d*%d", sys.sensor_num, sys.driver_num);
	...
	pr_err("find ini path:%s", ini_path);
	pr_err("This params is short_result_data = %s\n", short_result_data);
#endif
	...
	pr_err("save path is %s", save_path);
	...
	if (test_types & (_MAX_CHECK | _MIN_CHECK | _ACCORD_CHECK | _OFFSET_CHECK | _JITTER_CHECK | _KEY_MAX_CHECK | _KEY_MIN_CHECK | _UNIFORMITY_CHECK)) {
		...
		while (times < 16) {
			...
			if (ret < 0) {
				...
				pr_err("read rawdata timeout %d.", timeouts);
